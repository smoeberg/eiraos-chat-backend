...
class ChatCompletionRequest(BaseModel):
    """HTTP-neutral chat command contract; extra client fields are rejected."""
...
def _verification_failure(primary_answer: str, reason: str) -> VerificationResult:
    return VerificationResult(status="UNCERTAIN", reason=reason,
                               answer=primary_answer + VERIFICATION_FAILED_BADGE, verified=False)


class ChatExecutionService:
    """Application-level orchestration for chat execution.

    The HTTP controller is intentionally kept thin. Authorization dependencies
    remain at the API boundary, while execution concerns live here so budget,
    provider, verification and streaming policies can evolve independently.
    """

    async def execute(self, request: Request, payload: ChatCompletionRequest,
                      current_user: dict, org_id: int, db: AsyncSession):
        ...
        if not payload.stream:
            return await self._execute_non_streaming(
                request, payload, conversation, bot, provider, provider_messages,
                effective_system_prompt, idem_key, lease_token, db,
            )
    ...
    async def _execute_non_streaming(self, request, payload, conversation, bot, provider,
                                     provider_messages, effective_system_prompt, idem_key, lease_token, db):
        ...
        try:
            ...
            if payload.verify:
                try:
                    verifier_bot = await _find_verifier_bot(db, bot, payload.organization_id if hasattr(payload, "organization_id") else None)
                    if verifier_bot.organization_id is None:
                        raise RuntimeError("verifier organization unavailable")
                    verifier = await _provider_for_bot(verifier_bot, verifier_bot.organization_id)
    ...
    async def _event_generator(self, request, payload, conversation, bot, provider,
                               provider_messages, effective_system_prompt, idem_key, lease_token, db) -> AsyncIterator[str]:
        accumulated = ""
