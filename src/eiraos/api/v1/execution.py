...
@router.get("/cockpit")
async def get_cockpit(_org_id: int = Depends(get_current_active_organization)):
    return cockpit()
...
async def start_pipeline(workflow_id: str, _current_user: dict = Depends(require_execution_role), org_id: int = Depends(get_current_active_organization)):
    return await _transition(workflow_id, "start", org_id)

@router.post("/workflows/{workflow_id}/advance")
async def advance_pipeline(workflow_id: str, _current_user: dict = Depends(require_execution_role), org_id: int = Depends(get_current_active_organization)):
    return await _transition(workflow_id, "advance", org_id)

@router.post("/decisions/{decision_id}")
async def resolve_decision(decision_id: str, body: DecisionRequest, _current_user: dict = Depends(require_execution_role), org_id: int = Depends(get_current_active_organization)):
